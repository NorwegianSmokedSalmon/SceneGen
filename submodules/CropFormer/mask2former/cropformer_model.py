...
class CropFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for batched_input in batched_inputs:
            ...
            if "instances" in batched_input.keys():
                ...
                ori_instances.original_indices = torch.arange(
                    0, len(ori_instances)
                ).long()
        ...
        ## +1 means
        num_views = (
            self.cfg.ENTITY.CROP_SAMPLE_NUM_TRAIN + 1
            if self.training
            else self.cfg.ENTITY.CROP_SAMPLE_NUM_TEST + 1
        )
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                gt_instances = [
                    x["instances"].to(self.device) for x in batched_inputs_new
                ]
                targets_2d = self.prepare_targets_2d(
                    copy.deepcopy(gt_instances), copy.deepcopy(images)
                )
                targets_3d = self.prepare_targets_3d(
                    copy.deepcopy(gt_instances), copy.deepcopy(images), num_views
                )
            else:
                targets = None
            ...
            for k in list(losses_2d.keys()):
                if k in self.criterion_2d.weight_dict:
                    losses[k + "_2d"] = (
                        losses_2d[k] * self.criterion_2d.weight_dict[k] * 0.5
                    )
                else:
    ...
    def prepare_targets_3d(self, targets_ori, images, num_views):
        ...
        for count, targets in enumerate(new_targets):
            ...
            for v_i, targets_per_view in enumerate(targets):
                ...
                for i_i, (instance_mask, instance_valid) in enumerate(
                    zip(
                        targets_per_view.gt_masks.tensor,
                        targets_per_view.gt_boxes_valid,
                    )
                ):
                    if instance_valid == 1:
    ...
    def inference_whole_views(
        ...
        for view_index, (
            pred_masks_per_view,
            batched_input_per_view,
            image_size_per_view,
        ) in enumerate(zip(pred_masks, batched_inputs, image_sizes)):
            ...
            pred_masks_per_view = F.interpolate(
                pred_masks_per_view[None],
                size=(O_H, O_W),
                mode="bilinear",
                align_corners=False,
            )
        ...
        for crop_region_per_view, pred_masks_per_view in zip(
            crop_regions, new_pred_masks[1:]
        ):
