...
class Make3dQueries(nn.Module):
    ...
    def forward(self, output_2d, query_embed_2d, query_embed_3d):
        ...
        query_embed_2d = (
            query_embed_2d.unflatten(1, (B, T)).permute((0, 2, 1, 3)).flatten(0, 1)
        )

        for i in range(self.num_layers_3d):
            output_3d = self.enc_crosattn_3d[i](
                output_3d, output_2d, pos=query_embed_2d, query_pos=query_embed_3d
            )
...
class CropSharedMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        for i in range(self.num_feature_levels):
            ...
            pos_2d_, pos_3d_ = self.pe_layer(
                x[i].view(bs, t_m, -1, size_list[-1][0], size_list[-1][1])
            )
            ...
            pos_3d[-1] = (
                pos_3d[-1].view(bs, t_m, c, hw).permute(1, 3, 0, 2).flatten(0, 1)
            )
            src_3d[-1] = (
                src_3d[-1].view(bs, t_m, c, hw).permute(1, 3, 0, 2).flatten(0, 1)
            )
        ...
        for i in range(self.num_layers):
            ...
            attn_mask_2d[
                torch.where(attn_mask_2d.sum(-1) == attn_mask_2d.shape[-1])
            ] = False
            ...
            # FFN
            output_2d = self.transformer_ffn_layers[i](output_2d)
        ...
        out_2d = {
            ...
            "aux_outputs": self._set_aux_loss(
                predictions_class_2d if self.mask_classification else None,
                predictions_mask_2d,
            ),
        }
        ...
        for i in range(self.num_layers):
            level_index = i % self.num_feature_levels
            attn_mask_3d[
                torch.where(attn_mask_3d.sum(-1) == attn_mask_3d.shape[-1])
            ] = False
            ################# 3d (unified) #############
            # attention: cross-attention first
            ...
            output_3d = self.transformer_ffn_layers[i](output_3d)
        ...
        out_3d = {
            ...
            "aux_outputs": self._set_aux_loss(
                predictions_class_3d if self.mask_classification else None,
                predictions_mask_3d,
            ),
        }
    ...
    def forward_prediction_heads(
        self, output, mask_features, output_type, attn_mask_target_size
    ):
