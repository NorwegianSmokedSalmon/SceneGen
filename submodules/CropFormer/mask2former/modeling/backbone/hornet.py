...
if "DWCONV_IMPL" in os.environ:
    try:
        ...
        print(
            "[fail to use Megvii Large kernel] Using PyTorch large kernel dw conv impl"
        )
else:
...
class gnconv(nn.Module):
    def __init__(self, dim, order=5, gflayer=None, h=14, w=8, s=1.0):
        ...
        print(
            "[gconv]", order, "order with dims=", self.dims, "scale=%.4f" % self.scale
        )
...
class HorNet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(gnconv[0], str):
            ...
            # Python 3.12 inlines comprehensions; resolve the gnconv class in module scope.
            gnconv = [eval(g, globals()) for g in gnconv]
        ...
        for i in range(4):
            ...
            if self.use_checkpoint:
                x = checkpoint.checkpoint_sequential(
                    self.stages[i], len(self.stages[i]), x
                )
            else:
                x = self.stages[i](x)
